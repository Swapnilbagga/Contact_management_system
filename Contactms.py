...
def Numeric():
        if AGE.get().isdigit():
            ...
            result = msgbox.showwarning('', 'Age must be Numeric.', icon="warning")
        
        if FIRSTNAME.get().isdigit():
            result= msgbox.showwarning('','Firstname must be in alphabet.', icon="warning")
        else:
            FIRSTNAME.get()

        if LASTNAME.get().isdigit():
            result= msgbox.showwarning('','lastname must be in alphabet.', icon="warning")
        else:
            LASTNAME.get()
    
    
        if CONTACT.get().isdigit():
            CONTACT.get()
        else:
            result = msgbox.showwarning('', 'Contact Number  must be Numeric.', icon="warning")
...
def SubmitData():
    ...
    if  FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or AGE.get() == "" or ADDRESS.get() == "" or CONTACT.get() == "":
        result = msgbox.showwarning('', 'Please Complete The Required Field', icon="warning")
    else:
...
def DeleteData():
    if not tree.selection():
       result = msgbox.showwarning('', 'Please Select Something First!', icon="warning")
    else:
        result = msgbox.askquestion('', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
            contents =(tree.item(curItem))
            selecteditem = contents['values']
